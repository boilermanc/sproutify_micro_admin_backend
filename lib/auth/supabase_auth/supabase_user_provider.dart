...
class SproutifyMicroAdminSupabaseUser extends BaseAuthUser {
  SproutifyMicroAdminSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> sproutifyMicroAdminSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = SproutifyMicroAdminSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
