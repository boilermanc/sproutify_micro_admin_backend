...
class AdminMicroSupabaseUser extends BaseAuthUser {
  AdminMicroSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> adminMicroSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = AdminMicroSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
